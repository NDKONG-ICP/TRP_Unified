//! Sign-In with Sui (SIS) Canister
//! Verifies Sui signatures and maps Sui addresses to ICP principals
...
thread_local! {
    ...
    static SESSIONS: RefCell<StableBTreeMap<StorableString, SISSession, Memory>> = RefCell::new(
        StableBTreeMap::init(
...
fn format_sis_message(msg: &SISMessage) -> String {
    let mut message = format!("{} wants you to sign in with your Sui account:\n", msg.domain);
...
#[update]
fn verify_sis(message: SISMessage, signature: String) -> VerifyResult {
    ...
    let formatted_message = format_sis_message(&message);
    
    if !verify_sui_signature(&formatted_message, &signature, &message.address) {
        return VerifyResult::Err("Invalid signature".to_string());
    }
    
    let principal = derive_principal_from_address(&message.address);
    let session_id = format!("sis-{}-{}", message.address, ic_cdk::api::time());
    let now = ic_cdk::api::time();
    let expires_at = now + (7 * 24 * 60 * 60 * 1_000_000_000);
    
    let session = SISSession {
        session_id: session_id.clone(),
        sui_address: message.address.clone(),
        principal,
        created_at: now,
        expires_at,
    };
...
#[query]
fn get_session(session_id: String) -> Option<SISSession> {
    SESSIONS.with(|s| {
