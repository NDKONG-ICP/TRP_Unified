...
// Wallet interface with call method (proxy to management canister)
...
async function deployCanister(agent, wallet, managementActor, identity, canisterName, wasmPath) {
  ...
  try {
    ...
    // Try wallet.call to proxy through to management canister
    console.log(`   Creating canister via wallet proxy...`);
    try {
      // Encode create_canister arguments
      const createArgs = IDL.encode([IDL.Record({
        settings: IDL.Opt(IDL.Record({
          controllers: IDL.Opt(IDL.Vec(IDL.Principal)),
          compute_allocation: IDL.Opt(IDL.Nat),
          memory_allocation: IDL.Opt(IDL.Nat),
      ...
      canisterId = decoded.canister_id;
      console.log(`   ✓ Canister created via wallet proxy: ${canisterId.toText()}`);
    } catch (proxyError) {
      console.log(`   ⚠️  Wallet proxy failed: ${proxyError.message.substring(0, 150)}`);
      throw proxyError;
    }
...
async function main() {
  console.log('🚀 Deployment via Wallet Proxy');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
