...
const walletIDL = ({ IDL }) => IDL.Service({
  wallet_create_canister: IDL.Func(
    [IDL.Record({
      cycles: IDL.Nat64,  // Must be Nat64, not Nat or Int
...
async function deployCanister(agent, wallet, managementActor, identity, canisterName, wasmPath) {
  ...
  try {
    ...
    try {
      ...
      // Empty arrays [] represent None for opt types in @dfinity/agent
      // settings is REQUIRED record, inner fields are opt
      ...
      // If Actor validation fails, the error will be caught here
      // The issue is Actor's pre-validation, not the actual encoding
      const errorMsg = walletError.message || String(walletError);
      console.log(`   ⚠️  Wallet create failed: ${errorMsg.substring(0, 200)}`);
      console.log(`   💡 This is an Actor validation issue, not a wallet rejection`);
      console.log(`   💡 The wallet would accept this if we could bypass Actor validation`);
      throw walletError;
    }
...
async function main() {
  console.log('🚀 Deployment with CORRECT IDL Format');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
