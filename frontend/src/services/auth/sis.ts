...
const sisIdlFactory = ({ IDL }: any) => {
  ...
  const SISSession = IDL.Record({
    session_id: IDL.Text,
    sui_address: IDL.Text,
  ...
  return IDL.Service({
    verify_sis: IDL.Func([SISMessage, IDL.Text], [VerifyResult], []),
    get_session: IDL.Func([IDL.Text], [IDL.Opt(SISSession)], ['query']),
...
export function formatSISMessage(message: SISMessage): string {
  let formatted = `${message.domain} wants you to sign in with your Sui account:\n`;
...
export function createSISMessage(
  address: string,
  domain: string,
  uri: string,
...
export async function signInWithSui(
  domain: string,
  uri: string,
  chainId: string = 'mainnet'
): Promise<SISSession> {
  const connection = await connectSuiWallet();
  const { address } = connection.wallet;
  
  const message = createSISMessage(address, domain, uri, chainId, {
    statement: 'Sign in with Sui to the Raven Ecosystem',
  });
  ...
  const result = await (actor as any).verify_sis(backendMessage, signature) as SISVerifyResult;
  ...
  if (!result.Ok) {
    throw new Error('SIS verification failed');
  }
  
  return {
    sessionId: result.Ok.session_id,
    suiAddress: result.Ok.sui_address,
...
export async function getSISSession(sessionId: string): Promise<SISSession | null> {
  const actor = await createAuthActor('sis_canister', sisIdlFactory);
  
  const session = await (actor as any).get_session(sessionId);
  return session ? {
    sessionId: session.session_id,
    suiAddress: session.sui_address,
