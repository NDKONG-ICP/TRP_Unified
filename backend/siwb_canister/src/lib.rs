//! Sign-In with Bitcoin (SIWB) Canister
//! Verifies Bitcoin signatures and maps Bitcoin addresses to ICP principals
...
thread_local! {
    ...
    static SESSIONS: RefCell<StableBTreeMap<StorableString, SIWBSession, Memory>> = RefCell::new(
        StableBTreeMap::init(
...
fn format_siwb_message(msg: &SIWBMessage) -> String {
    let mut message = format!("{} wants you to sign in with your Bitcoin account:\n", msg.domain);
...
#[update]
fn verify_siwb(message: SIWBMessage, signature: String) -> VerifyResult {
    ...
    let formatted_message = format_siwb_message(&message);
    
    if !verify_bitcoin_signature(&formatted_message, &signature, &message.address) {
        return VerifyResult::Err("Invalid signature".to_string());
    }
    
    let principal = derive_principal_from_address(&message.address);
    let session_id = format!("siwb-{}-{}", message.address, ic_cdk::api::time());
    let now = ic_cdk::api::time();
    let expires_at = now + (7 * 24 * 60 * 60 * 1_000_000_000);
    
    let session = SIWBSession {
        session_id: session_id.clone(),
        bitcoin_address: message.address.clone(),
        principal,
        created_at: now,
        expires_at,
    };
...
#[query]
fn get_session(session_id: String) -> Option<SIWBSession> {
    SESSIONS.with(|s| {
